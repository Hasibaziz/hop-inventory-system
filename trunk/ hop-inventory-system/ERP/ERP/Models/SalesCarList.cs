using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

namespace ERP.Models
{
    public class SalesCarList
    {
        public string SL
        {
            get;
            set;
        }
        public string HaatDate
        {
            get;
            set;
        }
        public string Id
        {
            get;
            set;
        }
        public string Sellerid
        {
            get;
            set;
        }
        public string Brandid
        {
            get;
            set;
        }
        public string Catagoryid
        {
            get;
            set;
        }
        public string Carnameid
        {
            get;
            set;
        }
        public string Cc
        {
            get;
            set;
        }
        public string Color
        {
            get;
            set;
        }
        public string Sittingcap
        {
            get;
            set;
        }
        public string Manufyear
        {
            get;
            set;
        }
        public string Regyear
        {
            get;
            set;
        }
        public string Regno
        {
            get;
            set;
        }
        public string Aircon
        {
            get;
            set;
        }
        public string Powerstaring
        {
            get;
            set;
        }
        public string Cargear
        {
            get;
            set;
        }
        public string Enginetype
        {
            get;
            set;
        }
        public string Alloption
        {
            get;
            set;
        }
        public string Cng
        {
            get;
            set;
        }
        public string Cd
        {
            get;
            set;
        }
        public string Tv
        {
            get;
            set;
        }
        public string Backcamera
        {
            get;
            set;
        }
        public string Navigation
        {
            get;
            set;
        }
        public string Carprice
        {
            get;
            set;
        }
        public string Haatlocation
        {
            get;
            set;
        }
        public string Entrydate
        {
            get;
            set;
        }
        public string Modifydate
        {
            get;
            set;
        }
        public string Userid
        {
            get;
            set;
        }
        public string PictureUpdate
        {
            get;
            set;
        }
        public string PictureSource
        {
            get;
            set;
        }
        public string PictureMain
        {
            get;
            set;
        }
        public string PictureCount
        {
            get;
            set;
        }
        public string CarReview
        {
            get;
            set;
        }

        public string FilterBy
        {
            get;
            set;
        }

        public string SoldStatus
        {
            get;
            set;
        }

        public string BuyerID
        {
            get;
            set;
        }
        public string BuyerName
        {
            get;
            set;
        }
        public string TotalDeduct
        {
            get;
            set;
        }
        public string TotalDeposit
        {
            get;
            set;
        }
        public string TotalSelerPayment
        {
            get;
            set;
        }
        public string TotalBuyerPayment
        {
            get;
            set;
        }
        public string TotalBuyerDues
        {
            get;
            set;
        }
        public string StartDate
        {
            get;
            set;
        }
        public string EndDate
        {
            get;
            set;
        }            
            
    }
}